...
MANUAL = "https://drive.google.com/file/d/1jdWBG9VYvyMHwCTm8qOXZYiySCnZ531w/view?usp=drive_link"
KANA_TOOL = "https://tinyurl.com/kouza-kana"

# 添付ファイル名の先頭番号。書類の種類ごとに固定します。
FIXED_NO = {"seikyu": "1", "shiharai": "2", "juugyouin": "3",
            "software": "4", "nouhin": "5", "shashin": "6", "kouza": "7"}
...
NAV = [
    ...
    ("jisseki.html", "実績報告に必要な書類", [
        ...
        ("jisseki-juugyouin.html", "3 従業員一覧"),
        ("jisseki-software.html", "4 ソフトウェアの利用確認"),
        ("jisseki-hw-nouhin.html", "5 ハードウェアの納品書"),
        ("jisseki-hw-shashin.html", "6 ハードウェアの写真"),
        ("jisseki-kouza.html", "7 補助金の受取口座"),
    ]),
...
def manual_note():
    return note("公式のマニュアルもあります",
                ...
                f'<br><a href="{MANUAL}" target="_blank" rel="noopener">公式マニュアルを開く</a></p>')
...
<ul class="entry">
  ...
  <li><a href="tejun.html"><i>2</i><span><strong>申請マイページに入力して送信する</strong>
    <span>集めた書類を画面に添付し、口座情報を入力します。送信すると{COMPANY}に引き継がれます。</span>
    <em>目安：20〜30分</em></span></a></li>
  <li><a href="tejun-teishutsu.html"><i>3</i><span><strong>事務局へ提出する</strong>
    <span>{COMPANY}の確認が終わったあと、もう一度ログインして提出ボタンを押します。この操作で完了です。</span>
    <em>目安：5分</em></span></a></li>
</ul>

{warn("よくある取り違え", f"<p>2番の「送信」で終わったと思ってしまう方が多くいらっしゃいます。2番の時点では{COMPANY}に届いただけで、事務局には届いていません。必ず3番まで行ってください。3番のご案内は{COMPANY}からお送りします。</p>")}
...
{doclist([
 ("1", "jisseki-seikyusho.html", "請求書（請求明細書）", f"{COMPANY}が発行してお送りします。金額と宛名をご確認ください。"),
 ("2", "jisseki-shiharai.html", "支払証憑", "銀行振込が完了したことがわかる控えです。"),
 ("4", "jisseki-software.html", "ソフトウェアの利用確認", "導入したソフトが使える状態になっていることを示す画面の記録です。"),
 ("7", "jisseki-kouza.html", "補助金の受取口座", "補助金の振込先となる口座の、通帳またはネットバンキング画面です。"),
])}

{group("小規模事業者として申請した方だけ", "中小企業として申請した方は不要です・1点")}
{doclist([
 ("3", "jisseki-juugyouin.html", "従業員一覧", "従業員数の要件を満たしていることを示す一覧です。"),
])}

{group("パソコン・タブレット・POSレジ等を導入した方だけ", "ソフトウェアのみの方は不要です・2点")}
{doclist([
 ("5", "jisseki-hw-nouhin.html", "ハードウェアの納品書", "型番と数量がわかる納品書です。"),
 ("6", "jisseki-hw-shashin.html", "ハードウェアの写真", "現物と設置状況がわかる写真です。"),
])}
...
{table(["書類", "ファイル名の例"], [
 ...
 ("従業員一覧", "3_従業員一覧_株式会社サンプル.pdf"),
 ("ソフトウェア証憑", "4_ソフトウェア証憑_株式会社サンプル.pdf"),
 ("ハードウェア納品書", "5_ハードウェア導入情報（納品書）_株式会社サンプル.pdf"),
 ("ハードウェア写真", "6_ハードウェア導入情報（現物写真）_株式会社サンプル.pdf"),
 ("口座情報", "7_口座情報_株式会社サンプル.pdf"),
])}
...
{steps([
 ...
 ("複数枚を1つにまとめる",
  ...
  f"まとめ方が分からない場合は、撮影したファイルをそのまま{COMPANY}担当までお送りいただければ、こちらでまとめます。</p>"),
])}
...
<h1>請求書（請求明細書）</h1>
{summary(f"{COMPANY}からお送りする請求書を、そのままPDFで添付します。ご自身で作成する必要はありません。届いたら金額と宛名をご確認ください。")}
...
{warn("金額が違うとき", f"<p>交付決定額と請求額が異なる場合は、そのまま提出せず{COMPANY}担当までご連絡ください。別の手続きが必要になることがあります。</p>")}

<h2>提出前のチェック</h2>
{check([
 f"{COMPANY}から届いた請求書のPDFをパソコンに保存した",
...
# ---------------------------------------------------------------- 3 従業員一覧
PAGES["jisseki-juugyouin.html"] = ("3 従業員一覧", "小規模事業者として申請した場合に必要な従業員一覧。", f"""
<h1>従業員一覧</h1>
{summary("小規模事業者として申請した方だけが必要です。従業員の人数が要件内であることを示す一覧を作って提出します。")}
{filebox(FIXED_NO["juugyouin"], "3_従業員一覧_事業者名", "小規模事業者として申請した方のみ")}

{note("自分が小規模事業者かどうか分からないとき", f"<p>交付決定通知書または交付申請の控えに区分が記載されています。中小企業として申請していれば、この書類は不要です。判断がつかない場合は{COMPANY}担当までお問い合わせください。</p>")}

<h2>書き方</h2>
<p>書式の指定はありません。ExcelやWordで次の内容を並べ、PDFにして提出してください。</p>
{table(["列", "記載する内容"], [
 ("氏名", "常時使用する従業員の氏名（番号などでの管理でも可）"),
 ("雇用形態", "正社員・パート・アルバイトなど"),
 ("合計人数", "一覧の最後に合計を記載"),
])}
<p>賃金台帳や労働者名簿の写しでも差し支えありません。従業員がいない場合は「0名」と記載した一覧を提出してください。</p>

{note("人数の数え方", "<p>会社の代表者・役員、日雇いや契約期間が限られた方は、原則として人数に含めません。迷う場合はご相談ください。</p>")}
""")

# ---------------------------------------------------------------- 4 ソフトウェア
PAGES["jisseki-software.html"] = ("4 ソフトウェアの利用確認", "導入したソフトウェアが使える状態であることを示す資料。", f"""
<h1>ソフトウェアの利用確認</h1>
{summary("導入したソフトが「たしかに納品され、使える状態になっている」ことを示す画面の記録です。管理画面などを撮影してPDFにします。")}
{filebox(FIXED_NO["software"], "4_ソフトウェア証憑_事業者名", "全員が必要です")}
...
{note("画面の出し方が分からないとき", f"<p>ソフトによって確認できる画面が異なります。無理に進めず、{COMPANY}担当までご連絡ください。ご利用のソフトに合わせてご案内します。</p>")}

{manual_note()}
""")

# ---------------------------------------------------------------- 5 納品書
PAGES["jisseki-hw-nouhin.html"] = ("5 ハードウェアの納品書", "パソコン・POSレジ等を導入した場合の納品書。", f"""
<h1>ハードウェアの納品書</h1>
{summary("パソコン・タブレット・POSレジなどを導入した方だけが必要です。何を何台納品したかがわかる納品書を提出します。")}
{filebox(FIXED_NO["nouhin"], "5_ハードウェア導入情報（納品書）_事業者名", "ハードウェアを導入した方のみ")}
...
# ---------------------------------------------------------------- 6 写真
PAGES["jisseki-hw-shashin.html"] = ("6 ハードウェアの写真", "導入した機器の現物写真の撮り方。", f"""
<h1>ハードウェアの写真</h1>
{summary("買った機器が実際に届き、事業所で使える状態になっていることを写真で示します。3種類の写真を撮り、1つのPDFにまとめます。")}
{filebox(FIXED_NO["shashin"], "6_ハードウェア導入情報（現物写真）_事業者名", "ハードウェアを導入した方のみ")}
...
# ---------------------------------------------------------------- 7 口座
PAGES["jisseki-kouza.html"] = ("7 補助金の受取口座", "補助金が振り込まれる口座の資料と、名義の確認方法。", f"""
<h1>補助金の受取口座</h1>
{summary("補助金の振込先になる口座の情報です。この書類を見ながら、入力画面に口座情報を打ち込むことになります。")}
{filebox(FIXED_NO["kouza"], "7_口座情報_事業者名", "全員が必要です")}
...
{note("区分がわからないとき", f"<p>交付決定通知書に記載された枠と、申請時に選んだ事業者区分をご確認ください。判断がつかない場合は{COMPANY}担当までお問い合わせください。</p>")}
...
def build_tejun(kubun, pc, pos):
    ...
    files = [(no_seikyu, "1_請求明細書_事業者名"), (no_shiharai, "2_支払証憑_事業者名")]
    if shokibo:
        files.append((no_juugyouin, "3_従業員一覧_事業者名"))
    files.append((no_software, "4_ソフトウェア証憑_事業者名"))
    if hw:
        files.append((no_nouhin, "5_ハードウェア導入情報（納品書）_事業者名"))
        files.append((no_shashin, "6_ハードウェア導入情報（現物写真）_事業者名"))
    files.append((no_kouza, "7_口座情報_事業者名"))
